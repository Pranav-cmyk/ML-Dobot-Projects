...
def check_sys_and_update_dll():
    ...
    if currentsystem == 'Windows':
        ...
        for path in common_paths:
            temp_path = os.path.join(path, "MvCameraControl.dll")
            if os.path.exists(temp_path):
                if hasattr(os, 'add_dll_directory'):
                    os.add_dll_directory(path)
                    third_party = os.path.join(path, "ThirdParty")
                    if os.path.exists(third_party):
                        os.add_dll_directory(third_party)
                MvCamCtrldllPath = temp_path
                break

        # On 64-bit, WinDLL and CDLL are very similar, but some systems prefer one over the other
        # Using winmode=8 (LOAD_WITH_ALTERED_SEARCH_PATH) for absolute paths
        try:
